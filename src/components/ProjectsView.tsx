        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {isPM && (
            <button
              ...
              title="Importar grupo masivo de usuarios desde archivo CSV"
            >
      ...
      <div className="mt-6 mb-6 max-w-md">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar proyectos por nombre o clave (ej. PRO)..."
            className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl text-xs bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none shadow-2xs transition-all"
          />
        </div>
      </div>
          ...
          <p className="text-xs text-slate-500 mt-1.5 max-w-md mx-auto leading-relaxed">
            {searchTerm
              ...
              : `Hola ${currentUser?.name}, actualmente estás registrado como ${
                ...
                }. Aún no has sido asignado a ningún proyecto. Contacta al Project Manager o a la Product Owner.`}
          </p>
          {canManageProjects && !searchTerm && (
            <div className="mt-5 flex items-center justify-center gap-3">
              {isPM && (
                <button
                  onClick={() => setIsCsvModalOpen(true)}
                  className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold rounded-xl transition-colors shadow-2xs"
                >
                  <FileSpreadsheet className="w-4 h-4" />
              ...
              <button
                onClick={() => setIsCreateProjectOpen(true)}
                className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-xl transition-colors shadow-sm"
              >
                <Plus className="w-4 h-4" />
                Crear Proyecto
              </button>
